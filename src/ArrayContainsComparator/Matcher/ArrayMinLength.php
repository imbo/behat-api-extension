...
class ArrayMinLength
{
    ...
    public function __invoke(array $array, int|string $minLength): bool
    {
        ...
        if (!is_array($array)) {
            throw new InvalidArgumentException(sprintf('Only numerically indexed arrays are supported, got "%s".', gettype($array)));
        }
        ...
        if ($actualLength < $minLength) {
            throw new InvalidArgumentException(sprintf('Expected array to have more than or equal to %d entries, actual length: %d.', $minLength, $actualLength));
        }
